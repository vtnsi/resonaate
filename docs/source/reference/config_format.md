...
```python
<estimation_obj>: {
    "sequential_filter": { [<sequential_filter_obj>]
      "name": [<string>]  # Defaults to unscented_kalman_filter
      "parameters": {
        "alpha": [<decimal>],  # Defaults to 0.001
        "beta":  [<decimal>]  # Defaults to 2.0
      },
      "dynamics_model": [<string>],  # Default
      "maneuver_detection": {
        "name": [<string>],
        "maneuver_gate_val": [<decimal>],
        "parameters": {}
      },
    ...
    "adaptive_filter": {
      "name": [<string>],
      "orbit_determination": [<string>],
      "model_interval":  [<integer>],
      "stacking_method": [<string>],
      "observation_window": [<integer>],
      "prune_threshold":  [<decimal>],
      "prune_percentage":  [<decimal>]
    }
}
```
...
- `"sequential_filter"`
  ...
  - `"name"` defines which sequential filter class is used to track estimates.
    - `"unscented_kalman_filter"`: nonlinear Kalman filter which uses the Unscented Transform
  - `"parameters"` Dict of optional sequential filter parameters
    - `"alpha"` sigma point spread
    - `"beta"` Gaussian pdf parameter
  - `"dynamics_model"` Dynamics for sequential filter to use
  - `"maneuver_detection"` ManeuverDetection associated with the filter
    - `"name"` defines which maneuver detection method to use
    - `"maneuver_gate_val"` Threshold for Chi-squared maneuver detection test
    - `"parameters"` Dict of optional maneuver detection parameters
  - `"adaptive_estimation"` boolean indicating if adaptive estimation will be used

- `"adaptive_filter"`

  - `"name"` defines which adaptive filter class is used to track estimates.
  - `"orbit_determination"` String of OD method used
  - `"model_interval"`  time step between models
  - `"stacking_method"` String of stacking method used
  - `"observation_window"` Number of previous observations to look back to
  - `"prune_threshold"` probability that a model must be greater than to avoid pruning
  - `"prune_percentage"`  probability that a model must be greater than to assume convergence
...
- `"Optical"`: 2x2 measurement uncertainty
  ```python
  [[a ^ 2, ab], [ab, b ^ 2]]  # a: azimuth std in radians  # b: elevation std in radians
  ```
