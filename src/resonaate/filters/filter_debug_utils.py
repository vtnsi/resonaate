...
# Third Party Imports
import numpy as np
from scipy.linalg import norm, inv, cholesky
...
def checkThreeSigmaObs(current_obs, sigma=3):
    ...
    for observation in current_obs:
        ...
        true_measurements = [
            getAzimuth(ephem_sez),
            getElevation(ephem_sez)
        ]
        ...
        dist = mahalanobis(true_measurements, observation.measurements, inv(sensor_agent.sensors.r_matrix))
        ...
        if sez_diff > 1e-8 or dist > sigma or meas_diff > noise_limit:
            ...
            description["target_agent"].update({
                "ecef_state": target.ecef_state.tolist(),
                "time": target.time
            })
...
def innovationsCovarianceInflation(innovations_cov, r_matrix, observations, error_description):
    ...
    Raises:
        ``scipy.linalg.LinAlgError``: raised if innovations covariance matrix isn't invertible
            after inflating it
    """
    # Inflate innovations covariance
    innovations_cov += r_matrix * 1e3

    error_description["inflated"] = {
        "innov_cvr": innovations_cov.tolist(),
    ...
    # Try to invert, and let error propagate up if it fails
    inv(innovations_cov)

    # Otherwise return
    return innovations_cov, complete_filename
...
def createFilterDebugDict(filter_obj, description, observations, truth_state, sensor_agents):
