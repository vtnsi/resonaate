...
class TestOrbitElements(BaseTestCase):
    """Test orbital element set classes."""

    COE_SET = tuple(zip(SMA, ECC, INC, RAAN, ARGP, ANOM))
    ...
    def testConversions(self, sma, ecc, inc, raan, argp, anom):
        ...
        new_coe = ClassicalElements.fromEQE(
            eqe.sma, eqe.h, eqe.k, eqe.p, eqe.q, eqe.mean_longitude
        )
        assert eqe != coe
        assert new_coe == coe
    ...
    def testCOEFromConfig(self, monkeypatch, config):
    ...
    def testCOEFromConfigBadCOESet(self, monkeypatch):
    ...
    def testEQEFromConfig(self, monkeypatch, config):
