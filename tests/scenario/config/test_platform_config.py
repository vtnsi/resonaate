...
class TestPlatformConfig:
    ...
    def testCreation(
        ...
        mock_state.type = ECI_LABEL
        mock_state.position = [7000.0, 0.0, 0.0]

        with monkeypatch.context() as m_patch:
            m_patch.setattr(PlatformConfig, "valid_states", [ECI_LABEL])
            _ = PlatformConfig(
                type=platform_type,
                mass=mass,
                reflectivity=refl,
    ...
    def testCreationMismatchStateType(self, monkeypatch: pytest.MonkeyPatch):
        ...
        mock_state = MagicMock()
        mock_state.type = LLA_LABEL
        with monkeypatch.context() as m_patch:
            m_patch.setattr(PlatformConfig, "valid_states", [ECI_LABEL])
            with pytest.raises(ConfigValueError):
                _ = PlatformConfig(type=SPACECRAFT_LABEL, state=mock_state)


class TestSpacecraftConfig:
    """Test spacecraft platform type."""

    @pytest.mark.parametrize("state_type", [ECI_LABEL, EQE_LABEL, COE_LABEL])
    ...
    def testCreation(self, state_type: str, altitude: float):
        ...
        _ = SpacecraftConfig(
            type=SPACECRAFT_LABEL,
            state=mock_state,
        )

        _ = SpacecraftConfig.fromDict(
            {"type": SPACECRAFT_LABEL, "station_keeping": {"routines": []}}, state=mock_state
        )
    ...
    def testValidStates(self):
        ...
        assert cfg.valid_states == [ECI_LABEL, COE_LABEL, EQE_LABEL]


class TestGroundFacilityConfig:
    """Test ground facility platform type."""

    @pytest.mark.parametrize("state_type", [ECI_LABEL, LLA_LABEL])
    def testCreation(self, state_type: str):
        """Test creating ground facility config."""
        mock_state = create_autospec(StateConfig)
        mock_state.type = state_type
        _ = GroundFacilityConfig(
            type=GROUND_FACILITY_LABEL,
            state=mock_state,
        )

        _ = GroundFacilityConfig.fromDict({"type": GROUND_FACILITY_LABEL}, state=mock_state)
    ...
    def testValidStates(self):
        ...
        assert cfg.valid_states == [ECI_LABEL, LLA_LABEL]
