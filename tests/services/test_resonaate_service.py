...
class TestResonaateService(BaseTestCase):
    ...
    init_msg_importer = os.path.join(BaseTestCase.json_init_path, 'init_2018-12-1-12-0-0-importer.json')
    init_msg_realtime = os.path.join(BaseTestCase.json_init_path, 'init_2018-12-1-12-0-0-real.json')
    init_msg_later = os.path.join(BaseTestCase.json_init_path, 'init_2018-12-16-0-0-0.json')

    @pytest.fixture(scope="function", autouse=True)
    def fixtureSetupSericeApp(self, monkeypatch):
    ...
    def testRealtimePropagation(self, datafiles):
        ...
        init_message = InitMessage(
            ScenarioConfig.parseConfigFile(
    ...
    @pytest.mark.datafiles(FIXTURE_DATA_DIR)
    def testManualSensorTask(self, datafiles):
        ...
        database.resetData(tables=ResonaateDatabase.VALID_DATA_TYPES)
