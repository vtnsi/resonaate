...
def getCentralizedEngineClass(mocked_clock, mocked_sosi_network, mocked_filter, mocked_reward, mocked_decision):
    """Return reference to a minimal :class:`.Engine` class."""
    engine = CentralizedTaskingEngine(
        mocked_clock, mocked_sosi_network, mocked_filter, mocked_reward, mocked_decision, 1e-4
    )
...
class TestCentralizedTaskingEngine(BaseTestCase):
    ...
    def testExecuteTasking(self, centralized_tasking_engine):
        ...
        centralized_tasking_engine.executeTasking()
